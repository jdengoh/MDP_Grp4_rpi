...
class AndroidController(BaseController):
    ...
    def connect(self):
        self.logger.info('AC - Activating Bluetooth Connection')
        ...
        try:
            ...
            self.logger.info(f"AC - Waiting for connection on RFCOMM channel {port}")
            
            self.client_socket, client_info = self.server_socket.accept()
            self.logger.info(f"AC - Accepted connection from {client_info}")

        except Exception as e:
            self.logger.error(f"AC - Error establishing connection: {e}")
            self.server_socket.close()
            self.server_socket = None

    def disconnect(self):
        try:
            self.logger.info("AC - Deactivating Bluetooth Connection")
            ...
            self.server_socket = None
            self.client_socket = None
            self.logger.info("AC - Disconnected from Android")
    ...
    def send(self, msg) -> None:
        try:
            ...
            self.logger.debug(f"AC - Sent to Android: {msg.jsonify()}")
    ...
    def send_generic(self, msg: dict) -> None:
        try:
            ...
            self.logger.info(f"AC - Sent generic dict to Android: {msg}")
        except Exception as e:
            self.logger.error(f"AC - Error sending message to Android: {e}")
            raise e
    ...
    def receive(self) -> Optional[str]:
        try:
            ...
            self.logger.error(f"AC - Error receiving message from Android: {e}")
            raise e 
